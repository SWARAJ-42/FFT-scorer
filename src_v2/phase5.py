...
from utils.propensity import PropensityTable, rescore_poses
...
def run_phase5(
    ...
    interface_cutoff: float = 10.0,
    # ── NEW parameters ────────────────────────────────────────────────────
    prop_table:       "Optional[PropensityTable]" = None,  # fitted table or None
    prop_alpha:       float = 1.0,    # shape score weight
    prop_beta:        float = 0.5,    # propensity score weight
    prop_top_n:       int   = 2000,   # how many poses to rescore
    ...
    if prop_table is not None:
        print(f"  [propensity] Rescoring top {prop_top_n} poses "
              f"(α={prop_alpha}, β={prop_beta}) …")
        docking_results = rescore_poses(
            case    = case,
    ...
    print(f"{'═'*65}")
    _has_prop = prop_table is not None
    if _has_prop:
        print(f"  {'Rank':<6} {'Combined':>10} {'Shape':>10} {'Propensity':>11} "
              f"{'Contacts':>9} {'L-RMSD':>10} {'I-RMSD':>10}  Output")
        print(f"  {'----':<6} {'--------':>10} {'-----':>10} {'----------':>11} "
              f"{'--------':>9} {'------':>10} {'------':>10}  ------")
    else:
        # Original layout — unchanged when propensity is off
    ...
    for i, result in enumerate(docking_results[:top_n]):
        ...
        if _has_prop and hasattr(result, "_shape_score"):
            print(
                f"  {rank:<6} {result.score:>10.2f} "
                f"{result._shape_score:>10.2f} "
                f"{result._prop_score:>11.2f} "
                f"{result._n_contacts:>9d} "
                f"{lrmsd_str:>10} {irmsd_str:>10}  {rank_dir}"
            )
        else:
            print(
                f"  {rank:<6} {result.score:>10.2f} "
