...
from utils.electrostatics import ElecGridBuilder
...
def build_grids_for_case(case, builder=None, with_elec=False, elec_builder=None):
    """
    ...
    ----------
    case         : DockingCase
    builder      : GridBuilder (created with defaults if None)
    with_elec    : also build and return electrostatic grids
    elec_builder : ElecGridBuilder (created with defaults if None)
 
    Returns
    -------
    (pro_shape, rna_shape)
        if with_elec is False  (original behaviour — fully backwards-compatible)
 
    (pro_shape, rna_shape, pro_elec, rna_elec)
        if with_elec is True
    """
    if builder is None:
        from phase2 import GridBuilder
        builder = GridBuilder()
 
    pro_shape = builder.build(case.protein_struct, mol_type="protein")
    rna_shape = builder.build(case.rna_struct,     mol_type="rna")
 
    if not with_elec:
        return pro_shape, rna_shape          # unchanged return — safe for existing callers
 
    # Determine a shared spatial domain from the shape grid origins/dims
    # (we reuse the protein shape grid's origin as the shared origin)
    origin = pro_shape.origin
    dims   = pro_shape.grid_shape
 
    if elec_builder is None:
        elec_builder = ElecGridBuilder(
            resolution=builder.resolution,
            padding=builder.padding,
        )
 
    pro_elec = elec_builder.build(case.protein_struct, "protein", origin, dims)
    rna_elec = elec_builder.build(case.rna_struct,     "rna",     origin, dims)
 
    return pro_shape, rna_shape, pro_elec, rna_elec
