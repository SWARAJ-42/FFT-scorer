...
# Weight of the electrostatic FFT score relative to the shape score.
# Set to 0.0 to disable electrostatics entirely (reproduces original behaviour).
ELEC_WEIGHT: float = 0.3
...
from utils.electrostatics import ElecGridBuilder, build_elec_grids_for_case
...
class FFTDocker:
    ...
    def __init__(
        ...
        elec_weight:  float = ELEC_WEIGHT,   # ← NEW parameter
    ):
        self.resolution   = resolution
        self.elec_weight  = elec_weight       # ← NEW attribute
        self.sampler      = SO3Sampler(angular_step_deg=angular_step)
        self.grid_manager = CommonGridManager(resolution=resolution)
        self.elec_builder = ElecGridBuilder(resolution=resolution)  # ← NEW
        ...
        if elec_weight > 0:
            print(f"  Electrostatic scoring : ON  (weight={elec_weight})")
        else:
            print(f"  Electrostatic scoring : OFF  (elec_weight=0)")
    ...
    def dock(self, case) -> List[DockingResult]:
        ...
        if use_elec:
            ...
            pro_elec_arr     = pro_eg.elec_grid    # (Nx, Ny, Nz) float32
            rna_elec_arr_nat = rna_eg.elec_grid    # (Nx, Ny, Nz) float32
 
        # ── Push protein shape to GPU, precompute its FFT ────────────────
        pro_tensor = torch.tensor(pro_grid, dtype=torch.float32, device=self.device)
        fft_pro    = torch.fft.fftn(pro_tensor)
 
        # ── NEW: Push protein elec to GPU, precompute its FFT ───────────
        if use_elec:
            pro_elec_tensor = torch.tensor(
                pro_elec_arr, dtype=torch.float32, device=self.device
            )
            fft_pro_elec = torch.fft.fftn(pro_elec_tensor)
            rna_elec_native_gpu = torch.tensor(
                rna_elec_arr_nat, dtype=torch.float32, device=self.device
        ...
        for chunk_start in range(0, n_rots, ROTATION_BATCH_SIZE):
            ...
            for b in range(B):
                ...
                # ── FFT shape cross-correlation (GPU) ────────────────────
                fft_rna    = torch.fft.fftn(rna_rotated)
                corr_shape = torch.fft.ifftn(fft_pro * torch.conj(fft_rna)).real
 
                # ── NEW: FFT electrostatic cross-correlation (GPU) ────────
                if use_elec:
                    rna_elec_rot = rotate_grid_gpu(rna_elec_native_gpu, sg)
                    fft_rna_elec  = torch.fft.fftn(rna_elec_rot)
                    corr_elec     = torch.fft.ifftn(
                        fft_pro_elec * torch.conj(fft_rna_elec)
                    ).real
                    corr = corr_shape + self.elec_weight * corr_elec
                else:
                    corr = corr_shape
