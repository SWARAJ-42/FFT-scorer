...
def run_pipeline(args: argparse.Namespace) -> None:
    ...
    if args.propensity:
        ...
        if args.propensity_table and os.path.isfile(args.propensity_table):
    ...
    if args.multires:
        ...
        docker = MultiResDocker(
            ...
            coarse_res   = 2.0,
            fine_res     = args.resolution,
            top_k        = args.multires_topk,
        )
    else:
        # ── Standard single-resolution search (original behaviour) ────────
        docker = FFTDocker(
            angular_step = args.step,
            resolution   = args.resolution,
            elec_weight  = args.elec_weight,     # 0.0 = off, 0.3 = default
        )
    ...
    for idx, case in enumerate(cases, start=1):
        ...
        benchmarks = run_phase5(
            ...
            top_n            = args.top_n,
            prop_table       = prop_table,       # None if --propensity not set
            prop_beta        = args.prop_beta,
        )
...
Examples:
  ...
  python run.py --complex 1ASY --json "D:\\data\\PRDBv3_info.json" --pdb_root "D:\\data"
    ...
    parser.add_argument(
        ...
        default = False,
        help    = (
            "Enable multi-resolution hierarchical search "
            "(coarse 2 Å → fine 1 Å). Typically 4–6× faster than the "
            "full fine-resolution scan. Enabled with --multires flag."
        ),
    )
    parser.add_argument(
        "--multires-topk",
        ...
        default = 500,
        metavar = "K",
        help    = "Number of coarse-pass poses to refine at fine resolution "
                  "when --multires is active  (default: 500)",
    )
    parser.add_argument(
        "--elec-weight",
        type    = float,
        default = 0.3,
        metavar = "W",
        help    = "Weight of electrostatic FFT score relative to shape score. "
                  "Set to 0.0 to disable electrostatics  (default: 0.3)",
    )
    parser.add_argument(
        "--propensity",
        action  = "store_true",
        default = False,
        help    = "Enable interface propensity rescoring of FFT poses "
                  "before final ranking and PDB export.",
    )
    parser.add_argument(
        "--propensity-table",
        default = None,
        metavar = "FILE",
        help    = "Path to a pre-saved propensity JSON to load instead of "
                  "fitting from scratch (much faster).  "
                  "Generate with:  python propensity.py --save propensity.json",
    )
    parser.add_argument(
        "--prop-beta",
        type    = float,
        default = 0.5,
        metavar = "β",
        help    = "Propensity score weight in combined score "
                  "(combined = α×shape + β×propensity)  (default: 0.5)",
    )
    parser.add_argument(
        "--top_n",
        type    = int,
...
if __name__ == "__main__":
    ...
    print(f"  MultiRes      : {'ON  (coarse 2 Å → fine ' + str(args.resolution) + ' Å, top_k=' + str(args.multires_topk) + ')' if args.multires else 'OFF'}")
    print(f"  Electrostatics: {'ON  (weight=' + str(args.elec_weight) + ')' if args.elec_weight > 0 else 'OFF'}")
    print(f"  Propensity    : {'ON  (β=' + str(args.prop_beta) + ')' if args.propensity else 'OFF'}")
