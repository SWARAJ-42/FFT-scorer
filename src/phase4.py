...
class CommonGridManager:
    ...
    def build_rna_grid_native(self, struct: Structure, origin: np.ndarray, dims: Tuple[int, int, int]) -> Tuple[np.ndarray, np.ndarray]:
        """
...
def rotate_grid(grid: np.ndarray, R: np.ndarray) -> np.ndarray:
    """
...
class FFTDocker:
    ...
    def dock(self, case) -> List[DockingResult]:
        ...
        rna_grid_native, _ = self.grid_manager.build_rna_grid_native(case.rna_struct, origin, dims)
        ...
        for i, R in enumerate(self.sampler):
            ...
            # Rotate the grid itself via affine_transform
